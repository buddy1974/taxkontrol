...
export default function SuppliersPage() {
  ...
  async function handleAdd(e: React.FormEvent) {
    e.preventDefault()
    setSaving(true)
    const res = await fetch('/api/v1/payables', {
      method: 'POST',
    ...
    setShowForm(false)
    setSaving(false)
    setForm({ supplierName: '', description: '', totalAmount: '', dueDate: '' })
  }
  ...
  return (
    <div className="max-w-2xl space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-white">I owe suppliers</h1>
          <p className="text-gray-500 text-sm mt-1">
            Unpaid bills and outstanding obligations (Offene Verbindlichkeiten)
          </p>
        </div>
        <button
      ...
      <div className={`rounded-xl border p-5 ${
        ...
        <p className="text-xs text-gray-500 mt-1">
          {items.filter(i => i.status !== 'PAID').length} open bill(s)
        </p>
      </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                placeholder="Design subcontract"
                className="w-full bg-gray-800 border border-gray-700 text-white rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-blue-500"
              />
            </div>
            <div>
              ...
              <div className="flex items-start justify-between">
                <div>
                  <div className="flex items-center gap-2">
                    <p className="text-sm font-medium text-white">{item.supplierName}</p>
                ...
                <div className="mt-3">
                  {payingId === item.id ? (
                    <div className="flex gap-2">
                      ...
                      <button onClick={() => handlePay(item.id)} className="px-4 py-2 bg-red-600 hover:bg-red-500 text-white text-sm rounded-lg">
                        Mark as paid
                      </button>
